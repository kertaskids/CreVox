...
namespace Skill.Editor
{
    ...
    public class BatchMeshEditor : UnityEditor.Editor
    {
        ...
        void OnSceneGUI()
        {
            if (_Data.Sort == BatchMesh.SortAxis.Custom)
            {
                Vector3 preStart = _Data.SortAxisStart;
                Vector3 preEnd = _Data.SortAxisEnd;

                if (preStart == preEnd)
                {
                    preEnd = preStart + _Data.transform.forward * 5;
                }
                preStart = Handles.DoPositionHandle(preStart, Quaternion.identity);
                preEnd = Handles.DoPositionHandle(preEnd, Quaternion.identity);
                Handles.color = Color.cyan;
                Handles.ArrowCap(0, preStart, Quaternion.LookRotation(preEnd - preStart), HandleUtility.GetHandleSize(preStart));
                Handles.DrawLine(preStart, preEnd);

                Handles.SphereCap(0, preStart, Quaternion.identity, HandleUtility.GetHandleSize(preStart) * 0.2f);
                Handles.SphereCap(0, preEnd, Quaternion.identity, HandleUtility.GetHandleSize(preEnd) * 0.2f);

                if (preStart != _Data.SortAxisStart || preEnd != _Data.SortAxisEnd)
                {
                    _Data.SortAxisStart = preStart;
                    _Data.SortAxisEnd = preEnd;
        ...
        private void Generate()
        {
            ...
            foreach (var root in _Data.Roots)
            {
                ...
                {
                    if (fs[i].gameObject.activeInHierarchy && (Skill.Framework.MathHelper.GetLayerMask(fs[i].gameObject.layer) & _Data.Layers.value) != 0)
                        filters.Add(fs[i]);
                }
            }
            ...
            foreach (MeshGroup wu in units)
            {
                ...
                {
                    if (wu.Meshes.Count > 1)
                        GenerateGroups(wu.Meshes);
                    else
                    {
                        foreach (var m in wu.Meshes)
                            filters.Remove(m);
                    }
                }
            }
        ...
        class MeshMaterial
        {
            ...
            public void Sort(BatchMesh.SortAxis axis, Vector3 lineStart, Vector3 lineEnd)
            {
                MeshDataAxisComparer.Instance.SortAxis = axis;
                MeshDataAxisComparer.Instance.LineStart = lineStart;
                MeshDataAxisComparer.Instance.LineEnd = lineEnd;
            ...
            private class MeshDataAxisComparer : IComparer<MeshData>
            {
                ...
                public Vector3 LineStart
                {
                    ...
                    {
                        _LineStart = value;
                        _LineDirection = (_LineEnd - _LineStart).normalized;
                    }
                }
                ...
                public int Compare(MeshData x, MeshData y)
                {
                    switch (SortAxis)
                    {
                        ...
                        case BatchMesh.SortAxis.Custom:
                            float xD = Vector3.Distance(_LineStart, Skill.Framework.MathHelper.ProjectPointOnLine(x.Center, _LineStart, _LineDirection));
                            float yD = Vector3.Distance(_LineStart, Skill.Framework.MathHelper.ProjectPointOnLine(y.Center, _LineStart, _LineDirection));
        ...
        private void CreateMesh(MeshMaterial mm)
        {
            ...
            if (_Data.Sort != BatchMesh.SortAxis.None)
                mm.Sort(_Data.Sort, _Data.SortAxisStart, _Data.SortAxisEnd);
