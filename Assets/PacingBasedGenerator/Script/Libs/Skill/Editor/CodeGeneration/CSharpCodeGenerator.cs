...
namespace Skill.Editor.CodeGeneration
{
    ...
    public class CSharpCodeGenerator : ICodeGenerator
    {
        ...
        /// <summary>
        /// Generate code for SerializedData
        /// </summary>
        /// <param name="saveGame">SaveData</param>
        public void Generate(Skill.Editor.IO.SerializedData saveGame)
        {
            if (!string.IsNullOrEmpty(saveGame.Namespace))
            {
                Skill.Editor.IO.SerializedData.GeneratingInstance = saveGame;
                Namespace ns = new Namespace(saveGame.Namespace);
                SerializedDataClass sgClass = new SerializedDataClass(saveGame);                
                ns.Add(sgClass);
                _Document.AddUsingSkillIO();
                _Document.Add(ns);
                HasPartial = false;
                Skill.Editor.IO.SerializedData.GeneratingInstance = null;
            }
            else
            {
                Skill.Editor.IO.SerializedData.GeneratingInstance = saveGame;
