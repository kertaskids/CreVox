...
namespace Skill.Editor.CodeGeneration
{
    ...
    class ClassDataClass : Class
    {
            ...
            : base(saveClass.Name)
        {
            ...
            if (saveClass.Properties == null)
                saveClass.Properties = new PropertyData[0];
        ...
        private void CreateIsDirtyProperty()
        {
            ...
            foreach (var p in _SaveClass.Properties)
            {
                ...
                else if (p.Type == PropertyType.Reference)
                {
                    ReferencePropertyData rp = p as ReferencePropertyData;
                    if (rp.ReferenceType != SerializableReferenceType.Enum)
                    {
                        if (!hasClassProperty)
                        {
                            isDirtyBody.AppendLine("if(_IsDirty)  return _IsDirty;");
                        ...
                        if (p.IsArray)
                        {
                            isDirtyBody.AppendLine(string.Format("if({0} != null) {{", pName));
                            isDirtyBody.AppendLine(string.Format("for (int i = 0; i < {0}.Length; i++) {{", pName));
                            if (rp.ReferenceType == SerializableReferenceType.Class)
                                isDirtyBody.AppendLine(string.Format("if({0}[i] != null && {0}[i].IsDirty) return true;", pName));
                            else if (rp.ReferenceType == SerializableReferenceType.Struct)
                                isDirtyBody.AppendLine(string.Format("if({0}[i].IsDirty) return true;", pName));
                            isDirtyBody.AppendLine("}");
                            isDirtyBody.AppendLine("}");
                        }
        ...
        {
            if (SerializedData.GeneratingInstance != null)
            {
                foreach (var c in SerializedData.GeneratingInstance.Classes)
                {
                    if (c.Name == className)
                        return c;
                }
        ...
        private static string GetStaticCreatorMethodName(string className)
        {
            string cn = GetClassName(className);
            return string.Format("Create{0}", cn);
        }
        ...
        private string ConvertToString(PrimitiveDataType pt, bool safe)
        {
            switch (pt)
            {
                ...
                case PrimitiveDataType.UInt:
                    return safe ? "Skill.Framework.SafeUInt" : "uint";
                case PrimitiveDataType.Long:
                    return safe ? "Skill.Framework.SafeLong" : "long";
                case PrimitiveDataType.ULong:
                    return safe ? "Skill.Framework.SafeULong" : "ulong";
                default:
                    return pt.ToString();
            }
        }
            ...
            foreach (var p in _SaveClass.Properties)
            {
                switch (p.Type)
                {
                    case PropertyType.Primitive:
                        ...
                        if (p.IsArray)
                        {
                            Add(new Variable(string.Format("{0}[]", ConvertToString(pp.PrimitiveType, pp.SafeMemory)), p.Name, string.IsNullOrEmpty(p.InitialValue) ? null : p.InitialValue));
                        ...
                        {
                            Add(new Variable(ConvertToString(pp.PrimitiveType, pp.SafeMemory), p.Name, string.IsNullOrEmpty(p.InitialValue) ? null : p.InitialValue));
                    ...
                    case PropertyType.Reference:
                        ...
                        if (p.IsArray)
                        {
                            Add(new Variable(string.Format("{0}[]", rp.ClassName), p.Name));
                            Add(new Property(string.Format("{0}[]", rp.ClassName), p.Name, Variable.GetName(p.Name), true, Property.DirtyMode.CheckAndSet) { Comment = p.Comment });
                        }
                        else
                        {
                            Add(new Variable(rp.ClassName, p.Name));
                            Add(new Property(rp.ClassName, p.Name, Variable.GetName(p.Name), true, rp.ReferenceType != SerializableReferenceType.Class ? Property.DirtyMode.Set : Property.DirtyMode.CheckAndSet) { Comment = p.Comment });
                        }
                        break;
                    default:
                        break;
        ...
        private void CreateXmlSaveMethod()
        {
            ...
            foreach (var p in this._SaveClass.Properties)
            {
                ...
                else if (p.Type == PropertyType.Class)
                {
                ...
                {
                    if (((ReferencePropertyData)p).ReferenceType == SerializableReferenceType.Enum)
                    {
                        if (p.IsArray)
                            toXmlElementBody.AppendLine(string.Format("Skill.Framework.IO.XmlElement {0} = stream.Create(\"{1}\",Array.ConvertAll({2}, value => (int)value));", variable, p.Name, Variable.GetName(p.Name)));
                        else
                            toXmlElementBody.AppendLine(string.Format("Skill.Framework.IO.XmlElement {0} = stream.Create(\"{1}\",(int){2});", variable, p.Name, Variable.GetName(p.Name)));
                    }
                    else
                        toXmlElementBody.AppendLine(string.Format("Skill.Framework.IO.XmlElement {0} = stream.Create<{1}>(\"{2}\",{3});", variable, ((ReferencePropertyData)p).ClassName, p.Name, Variable.GetName(p.Name)));
                }
        ...
        private void CreateBinarySaveMethod()
        {
            ...
            foreach (var p in this._SaveClass.Properties)
            {
                ...
                else if (p.Type == PropertyType.Reference)
                {
                    ...
                    {
                        if (p.IsArray)
                            saveBinaryBody.AppendLine(string.Format("stream.Write(Array.ConvertAll({0}, value => (int)value));", Variable.GetName(p.Name)));
                        else
                            saveBinaryBody.AppendLine(string.Format("stream.Write((int){0});", Variable.GetName(p.Name)));
                    }
                    else
                        saveBinaryBody.AppendLine(string.Format("stream.Write<{0}>({1});", ((ReferencePropertyData)p).ClassName, Variable.GetName(p.Name)));
                }
            }
        ...
        private string GetLoadMethodName(PrimitiveDataType primitive, bool isArray, bool safe)
        {
            ...
            switch (primitive)
            {
                ...
                case PrimitiveDataType.Boolean:
                    ...
                    break;
                case PrimitiveDataType.UInt:
                    result = safe && isArray ? "ReadSafeUInt" : "ReadUInt";
                    break;
                case PrimitiveDataType.Long:
                    result = safe && isArray ? "ReadSafeLong" : "ReadLong";
                    break;
                case PrimitiveDataType.ULong:
                    result = safe && isArray ? "ReadSafeULong" : "ReadULong";
                    break;
                case PrimitiveDataType.String:
                    result = "ReadString";
                    break;
        ...
        private void CreateXmlLoadMethods()
        {
            ...
            if (_SaveClass.Properties.Length > 0)
            {
                ...
                foreach (var p in _SaveClass.Properties)
                {
                    ...
                    switch (p.Type)
                    {
                        ...
                        case PropertyType.Class:
                            ...
                            break;
                        case PropertyType.Reference:
                            ReferencePropertyData rp = (ReferencePropertyData)p;
                            if (rp.ReferenceType == SerializableReferenceType.Enum)
                            {
                                if (rp.IsArray)
                                    loadXmlBody.AppendLine(string.Format("this.{0} = Array.ConvertAll(stream.ReadIntArray(element), value => ({1})value);", Variable.GetName(p.Name), rp.ClassName));
                                else
                                    loadXmlBody.AppendLine(string.Format("this.{0} = ({2})stream.{1}( element );", Variable.GetName(p.Name), GetLoadMethodName(PrimitiveDataType.Integer, false, false), rp.ClassName));
                            }
                            else
                                loadXmlBody.AppendLine(string.Format("this.{0} = stream.{1}( element , {2}.{3} );", Variable.GetName(p.Name),
                                    GetSavableMethodName(rp.ClassName, rp.IsArray), rp.ClassName, GetStaticCreatorMethodName(rp.ClassName)));
                            break;
                    }
            ...
            foreach (var p in _SaveClass.Properties)
            {
                switch (p.Type)
                {
                    ...
                    case PropertyType.Reference:
                        ...
                        {
                            if (rp.IsArray)
                                loadMethodBody.AppendLine(string.Format("this.{0} = Array.ConvertAll(stream.ReadIntArray(), value => ({1})value);", Variable.GetName(p.Name), rp.ClassName));
                            else
                                loadMethodBody.AppendLine(string.Format("this.{0} = ({2})stream.{1}();", Variable.GetName(p.Name), GetLoadMethodName(PrimitiveDataType.Integer, false, false), rp.ClassName));
                        }
                        else
                            loadMethodBody.AppendLine(string.Format("this.{0} = stream.{1}( {2}.{3} );", Variable.GetName(p.Name),
                                GetSavableMethodName(rp.ClassName, rp.IsArray), rp.ClassName, GetStaticCreatorMethodName(rp.ClassName)));
