...
{
    public static class SerializedDataCompiler
    {
        private static bool _ErrorFound;
        private static SerializedData _SaveData;
        public static bool Compile(SerializedData data)
        {
            _ErrorFound = false;
            _SaveData = data;
            CheckForErrors(_SaveData);
            List<string> nameList = new List<string>(data.Classes != null ? data.Classes.Length : 0);
        ...
        private static void CheckForErrors(ClassData cl)
        {
            ...
            foreach (var p in cl.Properties)
            {
                ...
                if (p.Type == PropertyType.Class)
                {
                    ...
                    if (count <= 0)
                    {
                        Debug.LogError(string.Format("The property {0} of class {1} has invalid ClassName : '{2}'.", p.Name, cl.Name, ((ClassPropertyData)p).ClassName));
                        _ErrorFound = true;
                    }
                }
                ...
                if (p.Type == PropertyType.Reference)
                {
                    ...
                    if (string.IsNullOrEmpty(rp.ClassName))
                    {
