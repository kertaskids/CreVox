...
{
    [UnityEditor.CustomEditor(typeof(SerializedDataAsset))]
    public class SerializedDataDataEditor : UnityEditor.Editor
    {
        private SerializedDataAsset _Data;
        ...
        void OnEnable()
        {
            _Data = target as SerializedDataAsset;
            CreateUI();
        }
        ...
        {
            SerializedDataEditorWindow.Instance.Show();
            SerializedDataEditorWindow.Instance.Asset = _Data;
        }
        ...
        private void Build()
        {
            SerializedData saveData = _Data.Load();
            if (saveData != null)
            {
                bool compiled = SerializedDataCompiler.Compile(saveData);
                if (compiled)
