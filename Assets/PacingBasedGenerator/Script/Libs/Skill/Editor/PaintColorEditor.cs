...
namespace Skill.Editor
{
    ...
    public class PaintColorEditor : UnityEditor.Editor
    {
        ...
        public static PaintColorEditor Instance { get; private set; }


        #region UI

        private const int MaxFavColor = 8;
        ...
        private Skill.Editor.UI.ToggleButton _TbAutoSave;
        ...
        private Skill.Editor.UI.Slider _SliBrushScale;
        ...
        private FavoriteColor[] _FavoriteColors;
        class FavoriteColor : Grid
        {
            private Rectangle _Rect;
            private Box _SelectedBorder;
            ...
            public bool IsSelected
            {
                ...
                {
                    _IsSelected = value;
                    _SelectedBorder.Visibility = _IsSelected ? Visibility.Visible : Visibility.Hidden;
                }
            }
            public Color Color
            {
            ...
            public PaintColorEditor Editor { get; private set; }

            public FavoriteColor(PaintColorEditor editor, int index, Color color)
            {
                Editor = editor;
                Index = index;
                _Rect = new Rectangle() { Color = color, Margin = new Thickness(2) };
                _Button = new Button() { Style = new GUIStyle(), Tag = index.ToString() };
                _Button.Style.focused.background = _Button.Style.hover.background = _Button.Style.active.background = Skill.Editor.Resources.UITextures.Colors.Transparent;
                _SelectedBorder = new Box() { Style = Skill.Editor.Resources.Styles.SelectedItem };
                Controls.Add(_Rect);
                Controls.Add(_SelectedBorder);
        ...
        private void CreateUI()
        {
            ...
            if (_PaintColor.FavoriteColors == null || _PaintColor.FavoriteColors.Length != MaxFavColor)
            {
                _PaintColor.FavoriteColors = new Color[]
                {
            ...
            _PnlFavoriteColors.Controls.Add(new Box() { Row = 0, Column = 0, ColumnSpan = MaxFavColor });
            _FavoriteColors = new FavoriteColor[MaxFavColor];
            for (int i = 0; i < MaxFavColor; i++)
            {
                _PnlFavoriteColors.ColumnDefinitions.Add(1, GridUnitType.Star);
                _FavoriteColors[i] = new FavoriteColor(this, i, _PaintColor.FavoriteColors[i]) { Column = i };
                _PnlFavoriteColors.Controls.Add(_FavoriteColors[i]);
            }
            ...
            _SliBrushScale = new Skill.Editor.UI.Slider() { Value = _PaintColor.BrushScale, MinValue = 0.01f, MaxValue = 2, Margin = new Thickness(2), Height = 16 }; _SliBrushScale.Label.text = "Brush Scale";
            ...
            _TbShadowMode = new UI.ToggleButton() { Left = false, IsChecked = _PaintColor.ShadowMode, Margin = new Thickness(2), Height = 20 }; _TbShadowMode.Label.text = "Shadow Mode";
            _TbAutoSave = new UI.ToggleButton() { Left = false, IsChecked = _PaintColor.AutoSave, Margin = new Thickness(2), Height = 20 }; _TbAutoSave.Label.text = "AutoSave";
            ...
            _Panel.Controls.Add(_SliBrushScale);
            _Panel.Controls.Add(_TbShadowMode);
            _Panel.Controls.Add(_TbAutoSave);
            ...
            _TbAutoSave.Changed += _TbAutoSave_Changed;
            ...
            _SliBrushScale.ValueChanged += _SliBrushScale_ValueChanged;
            _BtnSaveTexture.Click += _BtnSaveTexture_Click;
        }



        private void SelectColor(FavoriteColor selectedColor)
        {
            foreach (var fc in _FavoriteColors)
                fc.IsSelected = false;
            if (selectedColor != null)
            {
                selectedColor.IsSelected = true;
                _ColorField.Color = selectedColor.Color;
            }
        }
        ...
        void _TbAutoSave_Changed(object sender, EventArgs e)
        {
            _PaintColor.AutoSave = _TbAutoSave.IsChecked;
            SetDirty2();
        }
        ...
        void _SliBrushScale_ValueChanged(object sender, EventArgs e)
        {
            _PaintColor.BrushScale = _SliBrushScale.Value;
            SetDirty2();
        }
        ...
        void _ColorField_ColorChanged(object sender, EventArgs e)
        {
            ...
            if (_FavoriteColors != null)
            {
                foreach (var fc in _FavoriteColors)
                {
                    if (fc.IsSelected)
                    {
                        fc.Color = _PaintColor.Color;
                        _PaintColor.FavoriteColors[fc.Index] = _PaintColor.Color;
                        break;
                    }
                }
            }
        ...
        {
            if (_Changed && _PaintColor.AutoSave)
            {
                SaveTexture();
                _Changed = false;
            }
        ...
        private void DrawHandle()
        {
            ...
            if (Physics.Raycast(ray, out hit, 1000f, _PaintColor.LayerMask))
            {
                ...
                Handles.CircleCap(0, hit.point, rotation, _PaintColor.Radius * _PaintColor.BrushScale);
        ...
        private void UpdatePreviewBrush()
        {
            ...
            float m_Size = _PaintColor.Radius * _PaintColor.BrushScale;
            float num = 1f;
