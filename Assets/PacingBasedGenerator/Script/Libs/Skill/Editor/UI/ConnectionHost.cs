...
namespace Skill.Editor.UI
{
    ...
    public class ConnectionHost : Grid
    {
            ...
            if (_IsDetecting && Parent != null && e != null)
            {
                ...
                else if ((e.type == EventType.MouseUp || e.rawType == EventType.MouseUp) && e.button == 0)
                {
                    ...
                    if (of != null)
                    {
                        ...
                        {
                            if (c.ConnectorType != _StartConnector.ConnectorType && c.RenderArea.Contains(mousePosition) && !c.IsConnectedTo(_StartConnector))
                            {
                                if (CanConnect(_StartConnector, c))
                                {
        ...
        protected override void Render()
        {
            ...
            if (_IsDetecting)
            {
                Connection.DrawConnection(this._StartConnector.ConnectionPoint, this._EndPreviewConnector, this.PreviewColor, this.PreviewNumSegments);
            }
        }
