        ...
        protected override void Render()
        {
            ...
            if (ShowTimePosition)
            {
                if (TimeLine.TimePosition >= TimeLine.StartVisible && TimeLine.TimePosition < TimeLine.EndVisible)
                {
                    ...
                    Skill.Editor.LineDrawer.DrawLine(new Vector2(x, ra.yMin), new Vector2(x, ra.yMax - ScrollbarThickness), ThumbColor);
                }

                x = ra.x + ScrollbarThickness + (float)(((TimeLine.TimePosition - TimeLine.MinTime) / (TimeLine.MaxTime - TimeLine.MinTime)) * width2);
                Skill.Editor.LineDrawer.DrawLine(new Vector2(x, ra.yMax - ScrollbarThickness + 2), new Vector2(x, ra.yMax - 1), ThumbColor);
            }
            #endregion
