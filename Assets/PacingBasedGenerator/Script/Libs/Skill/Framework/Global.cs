...
namespace Skill.Framework
{
    ...
    public class Global : StaticBehaviour
    {
        ...
        /// <param name="clip">Sound to play</param>
        /// <param name="category">Category of sound</param>
        /// <param name="volumeFactor">additional volume factor</param>
        public virtual void PlayOneShot(AudioSource source, AudioClip clip, Skill.Framework.Audio.SoundCategory category, float volumeFactor)
        {
            if (source != null && clip != null)
            {
                float volume = Settings.Audio.GetVolume(category) * volumeFactor;
                if (volume > 0)
                    source.PlayOneShot(clip, volume);
            }
        }
        ...
        /// <summary> Occurs when a Settings.Quality changed </summary>
        public static event System.EventHandler QualityChanged;

        /// <summary>
        /// Occurs when a Settings.Quality changed 
        /// </summary>
        /// <param name="sender"> The source of the event. </param>        
        public static void RaiseQualityChanged(object sender)
        {
            if (QualityChanged != null)
                QualityChanged(sender, System.EventArgs.Empty);
        }
    }
}
