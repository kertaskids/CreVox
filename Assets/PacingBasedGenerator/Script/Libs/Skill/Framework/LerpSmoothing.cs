...
namespace Skill.Framework
{
    ...
    public struct LerpSmoothing
    {
        ...
        public float Update(float smoothing, bool useRealTime = false)
        {
            if (_Current != Target)
            {
                if (useRealTime)
                    _Current = Mathf.Lerp(_Current, _Target, smoothing * Time.unscaledDeltaTime);
                else
    ...
    public struct LerpSmoothing2D
    {
        ...
        /// <param name="smoothing">Smoothing</param>
        /// <returns>Smooth result</returns>
        public Vector2 Update(float smoothing, bool useRealTime = false)
        {
            if (_Current != Target)
            {
                if (useRealTime)
                    _Current = Vector2.Lerp(_Current, _Target, smoothing * Time.unscaledDeltaTime);
                else
    ...
    public struct LerpSmoothing3D
    {
        ...
        /// <param name="smoothing">Smoothing</param>
        /// <returns>Smooth result</returns>
        public Vector3 Update(float smoothing, bool useRealTime = false)
        {
            if (_Current != Target)
            {
                if (useRealTime)
                    _Current = Vector3.Lerp(_Current, _Target, smoothing * Time.unscaledDeltaTime);
                else
    ...
    public struct LerpSmoothing4D
    {
        ...
        /// <param name="smoothing">Smoothing</param>
        /// <returns>Smooth result</returns>
        public Vector4 Update(float smoothing, bool useRealTime = false)
        {
            if (_Current != Target)
            {
                if (useRealTime)
                    _Current = Vector4.Lerp(_Current, _Target, smoothing * Time.unscaledDeltaTime);
                else
    ...
    public struct LerpSmoothingColor
    {
        ...
        /// <param name="smoothing">Smoothing</param>
        /// <returns>Smooth result</returns>
        public Color Update(float smoothing, bool useRealTime = false)
        {
            if (_Current != Target)
            {
                if (useRealTime)
                    _Current = Color.Lerp(_Current, _Target, smoothing * Time.unscaledDeltaTime);
                else
    ...
    public struct LerpSmoothingAngle
    {
        ...
        public float Update(float smoothing, bool useRealTime = false)
        {
            ...
            {
                if (useRealTime)
                    _CurrentAngle = Mathf.LerpAngle(_CurrentAngle, _TargetAngle, smoothing * Time.unscaledDeltaTime);
                else
