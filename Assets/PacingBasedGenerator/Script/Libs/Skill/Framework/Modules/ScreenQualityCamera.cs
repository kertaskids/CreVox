...
namespace Skill.Framework.Modules
{
    ...
    public class ScreenQualityCamera : DynamicBehaviour
    {
        ...
        {
            base.HookEvents();
            Global.QualityChanged += Global_QualityChanged;
        }
        ...
        void Global_QualityChanged(object sender, System.EventArgs e)
        {
            Skill.Framework.Modules.ScreenQualityCamera.Instance.Apply();
        }
        ...
        protected override void Awake()
        {
            ...
            _ReallySupportsRenderTarget = _SupportsRenderTarget = SystemInfo.supportsImageEffects; //SystemInfo.supportsRenderTextures && SystemInfo.supportsImageEffects;
            _Rebuild = true;
            _Apply = true;
        ...
        private void Rebuild(float quality)
        {
            ...
            else
            {
                ...
                this.Width = Mathf.Max(Mathf.Min(Mathf.FloorToInt(Screen.width * _PreQuality), UnityEngine.SystemInfo.maxTextureSize), 256);
                this.Height = Mathf.Max(Mathf.Min(Mathf.FloorToInt(Screen.height * _PreQuality), UnityEngine.SystemInfo.maxTextureSize), 256);

                if (Aspect > 1)
                    this.Height = Mathf.FloorToInt((float)this.Width / Aspect);
