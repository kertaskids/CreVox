        ...
        {            
            ...
            if (State == WeaponState.Ready)
            {
                if (_RequestReload && (SelectedProjectile.Ammo > 0 || SelectedProjectile.InfinitClip || SelectedProjectile.InfinitAmmo))
                {
                    ...
                    if (completeReload)
                        _BusyTW.Begin(SelectedProjectile.CompleteReloadTime, IgnoreTimeScale);
                    else
                        _BusyTW.Begin(SelectedProjectile.ReloadTime, IgnoreTimeScale);
                ...
                {
                    _BusyTW.Begin(_RequestBusy, IgnoreTimeScale);
                    State = WeaponState.Busy;
                    _RequestBusy = 0;
                }
                else if (_SwitchProjectileIndex != _SelectedProjectileIndex)
                {
                    _BusyTW.Begin(SelectedProjectile.EquipTime, IgnoreTimeScale);
                ...
                else if (IsFiring)
                {
                    ...
                    else
                    {
                        ...
                        {
                            _BusyTW.Begin(SelectedProjectile.FireInterval, IgnoreTimeScale);
